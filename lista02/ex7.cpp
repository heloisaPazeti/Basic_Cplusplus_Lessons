...
class BigInt {
    ...
    public:
        ...
        BigInt operator/(int b) {
            ...
            BigInt temp = BigInt(this->_npartes, false);                                       //  Cria temporario
            if(this->_positivo != (b >= 0)) temp._positivo = false;                            // Altera sinal

            for (i = 0; i < this->_npartes; i++)
            {
                temp._partes[i] = (this->_partes[i]+res) / std::abs(b);                        // Coloca a divisao
                res = ((this->_partes[i]+res) % b)*max;                                        // Passa o resto
            }

            for(int i = 0; i < this->_npartes-1; i++)                                          // Remove os zeros
            {
                if(this->_partes[i] == 0)
                    leftZero++;
                else
       ...
       friend std::ostream& operator<<(std::ostream &out, const BigInt &a) {
        ...
        if(a._partes == nullptr)                                                // Checa se eh nulo
            return out;

        if(a == BigInt())                                                       // Checa se eh zero
        {
            out << '0';
            return out;
        }

        if(!a._positivo)                                                        // Checa se eh negativo
        {
            if(!(a._npartes == 1 && a._partes[0] == 0))
                out << "-";
        }

        for(i = 0; i < a._npartes-1; i++)                                       // Nao printa zeros a esquerda
        {
            if(a._partes[i] != 0)
                break;
        }

        while(i < a._npartes)                                                   // Printa o restante
        {
            ...
            if(isFirst)
                ...
                temp.insert(0, 9 - temp.size(), '0');                           // Completa com zeros caso precise
...
class Fracao {
    ...
    public:
        ...
        friend Fracao operator+(const Fracao& a, const Fracao& b) {
                ...
                Fracao result;
                result = Fracao((a._numerador*b._denominador) + (a._denominador * b._numerador), a._denominador * b._denominador);

                if(result._numerador == BigInt())
                    return Fracao();
